...
public class GoogleSignInController {
    ...
    public init(config: GoogleSignInConfig, session: GoogleSignInURLSessionProtocol) {
        self.config = config
    ...
    public func getTokenResponse(using redirectUrl: URL, completion: @escaping (Result<GoogleSignInTokenResponse, GoogleSignInError>) -> Void) {
    ...
    private let config: GoogleSignInConfig
    private let session: GoogleSignInURLSessionProtocol
    ...
    private func decodeToken(from data: Data) throws -> GoogleSignInTokenResponse {
        let decoder = JSONDecoder()
        decoder.keyDecodingStrategy = .convertFromSnakeCase
        return try decoder.decode(GoogleSignInTokenResponse.self, from: data)
    }
